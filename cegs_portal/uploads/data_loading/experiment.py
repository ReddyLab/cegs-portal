...
class Experiment:
    ...
    def load(self):
        ...
        genome_assembly = self.tested_elements_file.genome_assembly

        new_elements: dict[str, FeatureRow] = {}
        new_parent_elements: dict[str, FeatureRow] = {}

        element_file = self.tested_elements_file
        element_cell_line = element_file.biosample.cell_line

        parent_file = self.tested_elements_parent_file
        parent_cell_line = parent_file.biosample.cell_line

        element_tsv = open(element_file.filename, "r")
        ...
        for line in reader:
            ...
            if parent_name not in new_parent_elements:
                new_parent_elements[parent_name] = FeatureRow(
                    ...
                    genome_assembly=GenomeAssembly(parent_file.genome_assembly),
                    cell_line=parent_cell_line,
            ...
            categorical_facets = [f.split("=") for f in line["facets"].split(";")]
            misc = {f.split("=") for f in line["facets"].split(";")}
        ...
        element_tsv.close()
        self.features, self.parent_features = new_elements.values(), new_parent_elements.values()
        return self
