...
from cegs_portal.search.views.v1 import TargetEffectsView
...
@pytest.fixture
def view():
    return TargetEffectsView.as_view()
...
def test_target_reg_effects_list_e2e(client: Client, target_reg_effects, sig_only_target_reg_effects):
    ...
    response = client.get(f"/search/feature/accession/{target.accession_id}/target_of")

    assert response.status_code == 200


def test_sig_only_target_reg_effects(public_test_client: RequestBuilder, view, sig_only_target_reg_effects):
...
def test_sig_only_false_target_reg_effects_list_json(
    public_test_client: RequestBuilder, view, sig_only_target_reg_effects
):
    target = sig_only_target_reg_effects["target"]
    response = public_test_client.get(
...
def test_hidden_target_reg_effects_list_json(public_test_client: RequestBuilder, view, hidden_target_reg_effects):
    target = hidden_target_reg_effects["target"]
    response = public_test_client.get(
        f"/search/feature/accession/{target.accession_id}/target_of?accept=application/json"
    ).request(view, feature_id=target.accession_id)
...
def test_get_target_reg_effects_with_anonymous_client(
...
def test_get_archived_target_reg_effects_with_anonymous_client(
