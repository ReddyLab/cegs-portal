...
def test_experiment_list_json(
    public_test_client: RequestBuilder, experiment_list_view, experiment_list_data: tuple[Any, Any, Any, Any, Any, Any]
):
    experiments, igvf_experiments, collections, _, _, _ = experiment_list_data
    ...
    assert len(json_content["igvf_experiments"]) == len(igvf_experiments)
    ...
    for json_expr, expr in zip(json_content["igvf_experiments"], igvf_experiments):
        assert json_expr["accession_id"] == expr.accession_id
        assert json_expr["name"] == expr.name
        assert json_expr["description"] == (expr.description if expr.description is not None else "")
...
def test_experiment_list_facet_json(
    ...
    _, _, _, facets, _, _ = experiment_list_data
    ...
    assert len(json_content["experiments"]) == 4


def test_igvf_experiment_list_facet_json(
    public_test_client: RequestBuilder, experiment_list_view, experiment_list_data: tuple[Any, Any, Any, Any, Any, Any]
):
    _, _, _, _, facets, _ = experiment_list_data
    response = public_test_client.get(f"/search/experiment?accept=application/json&igvf_facet={facets[0].id}").request(
        experiment_list_view
    )
    assert response.status_code == 200
    json_content = response.json()

    assert len(json_content["igvf_experiments"]) == 1


def test_experiment_collection_list_facet_json(
    public_test_client: RequestBuilder, experiment_list_view, experiment_list_data: tuple[Any, Any, Any, Any, Any, Any]
):
    _, _, _, _, _, facets = experiment_list_data
